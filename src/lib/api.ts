...
// Most list edge functions respond with { data: [...], count } (or { data: [...] }),
// which callEdgeFunction wraps one more level. Unwrap to the plain array so screens
// receive a list they can render directly.
async function unwrapList(name: string, params?: Record<string, any>): Promise<{ data: any[] | null; error: any }> {
  const res = await callEdgeFunction<any>(`${name}${queryString(params)}`);
...
export const api = {
  ...
  listPermissions: async () => unwrapList("admin-permissions-list"),
  ...
  listAdmins: async (status?: string) => unwrapList("owner-admins-list", status ? { status } : undefined),
  ...
  listTournaments: async (params?: { status?: string; q?: string; limit?: number; offset?: number }) =>
    unwrapList("admin-tournaments-list", params),
  ...
  listPresets: async () => unwrapList("admin-tournaments-preset?action=list"),
  ...
  getEntrants: async (tournament_id: string) => {
    const res = await callEdgeFunction<any>(`admin-tournaments-entrants?tournament_id=${tournament_id}`);
    if (res.error || !res.data) return { data: null, error: res.error };
    return { data: { entrants: res.data.registrations ?? [], rosters: res.data.rosters ?? [], unassigned_count: res.data.unassigned_count ?? 0 }, error: null };
  },
  ...
  getResults: async (tournament_id: string) => {
    const res = await callEdgeFunction<any>(`admin-results-get?tournament_id=${tournament_id}`);
    if (res.error || !res.data) return { data: null, error: res.error };
    return { data: Array.isArray(res.data) ? res.data : (res.data?.data ?? []), error: null };
  },
  ...
  listTopups: async (params?: { status?: string; method?: string; risk?: string; limit?: number; offset?: number }) =>
    unwrapList("admin-topups-list", params),
  ...
  listWithdrawals: async (params?: { status?: string; limit?: number; offset?: number }) =>
    unwrapList("admin-withdrawals-list", params),
  ...
  listRewardCampaigns: async () => unwrapList("admin-rewards-campaigns"),
  ...
  searchPlayers: async (q: string) => unwrapList("admin-players-search", { q }),
  ...
  listBanners: async () => unwrapList("admin-content-banners"),
  ...
  queryAudit: async (params?: { action?: string; actor_id?: string; entity_type?: string; limit?: number; offset?: number; since?: string }) =>
    unwrapList("admin-audit-query", params),
