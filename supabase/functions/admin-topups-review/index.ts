...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (topup.profile_id === user.id) return withCors(req, jsonError("FORBIDDEN" as any, "Cannot review own request", 403));
    ...
    if (decision === "approve") {
      ...
      const { error: creditErr } = await admin.rpc("wallet_credit", { p_profile_id: topup.profile_id, p_amount: topup.amount_coins, p_type: "topup_approved", p_reference_type: "topup", p_reference_id: id, p_idempotency_key: key, p_created_by: user.id } as any);
      if (creditErr) return withCors(req, jsonError("INTERNAL" as any, creditErr.message, 500));
      await admin.schema("app").from("topup_requests").update({ status: "approved", reviewed_by: user.id, reviewed_at: new Date().toISOString() }).eq("id", id);
      ...
      await writeAuditLog({ actorId: user.id, action: "topup.approve", entityType: "topup", entityId: id, after: { amount: topup.amount_coins } });
    } else {
      await admin.schema("app").from("topup_requests").update({ status: "rejected", reviewed_by: user.id, reviewed_at: new Date().toISOString(), reject_reason: reason }).eq("id", id);
      ...
      await writeAuditLog({ actorId: user.id, action: "topup.reject", entityType: "topup", entityId: id, reason });
    }
