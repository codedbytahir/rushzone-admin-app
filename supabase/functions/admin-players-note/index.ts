...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: inserted } = await admin.schema("app").from("internal_notes").insert({ profile_id: profileId, author_id: userData.user.id, body: bodyText }).select("id, created_at").single();
    await writeAuditLog({ actorId: userData.user.id, action: "player.note.add", entityType: "profile", entityId: profileId, after: { note_id: inserted.id } });
