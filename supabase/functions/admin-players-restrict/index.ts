...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (lift) {
      ...
      await writeAuditLog({ actorId: userData.user.id, action: "player.restrict.lift", entityType: "profile", entityId: profileId, after: { type } });
      if (type === "suspend" || type === "ban") await admin.schema("app").from("profiles").update({ status: "active" }).eq("id", profileId);
    } else {
      await admin.schema("app").from("restrictions").insert({ profile_id: profileId, type, reason, applied_by: userData.user.id, expires_at: expiresAt } as any);
      await writeAuditLog({ actorId: userData.user.id, action: "player.restrict." + type, entityType: "profile", entityId: profileId, reason, after: { type, expires_at: expiresAt } });
