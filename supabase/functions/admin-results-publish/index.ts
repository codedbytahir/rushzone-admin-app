...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: publishedCount, error } = await admin.rpc("publish_results", { p_tournament_id: tournamentId, p_published_by: userData.user.id } as any);
    ...
    await writeAuditLog({ actorId: userData.user.id, action: "result.publish", entityType: "tournament", entityId: tournamentId, after: { published: publishedCount } });
