...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await admin.schema("app").from("settings").upsert({ key: "referral_config", value: config, updated_by: userData.user.id } as any);
    await writeAuditLog({ actorId: userData.user.id, action: "referral.config.update", entityType: "settings", entityId: "referral_config", after: config });
