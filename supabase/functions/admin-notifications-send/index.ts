...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (isBroadcast) {
      ...
      await writeAuditLog({ actorId: userData.user.id, action: "notification.broadcast", entityType: "notification", entityId: "broadcast", after: { title, type } });
    ...
    if (tournamentId) {
      ...
      await writeAuditLog({ actorId: userData.user.id, action: "notification.tournament", entityType: "tournament", entityId: tournamentId, after: { title } });
    ...
    await writeAuditLog({ actorId: userData.user.id, action: "notification.send", entityType: "profile", entityId: profileId, after: { title } });
