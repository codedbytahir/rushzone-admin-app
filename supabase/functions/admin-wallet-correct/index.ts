...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let newBal: any = null;
    if (direction === "credit") {
      const { data, error } = await admin.rpc("wallet_credit", { p_profile_id: profileId, p_amount: amount, p_type: "admin_correction", p_reference_type: "correction", p_reference_id: null, p_idempotency_key: key, p_created_by: user.id, p_note: reason } as any);
      if (error) return withCors(req, jsonError("INTERNAL" as any, error.message, 500));
      newBal = data;
    } else {
      const { data, error } = await admin.rpc("wallet_debit", { p_profile_id: profileId, p_amount: amount, p_type: "admin_correction", p_reference_type: "correction", p_reference_id: null, p_idempotency_key: key, p_created_by: user.id, p_note: reason } as any);
      if (error) return withCors(req, jsonError("INTERNAL" as any, error.message, 500));
      newBal = data;
    }
    await writeAuditLog({ actorId: user.id, action: "wallet.correct", entityType: "profile", entityId: profileId, reason, after: { direction, amount, balance_after: newBal } });
