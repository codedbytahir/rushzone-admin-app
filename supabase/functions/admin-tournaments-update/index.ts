...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { user } = await requireAdmin(req, "tournament.publish");
    ...
    const allowed = ["title","description","internal_notes","mode","map","cover_path","rounds","capacity","entry_fee","prize_pool","prize_distribution","score_rules","rules_text","reg_open_at","reg_close_at","match_start_at","room_release_at","result_expected_at","free_slot_enabled","free_slot_trigger","status"];
    const update: any = {};
    ...
    await writeAuditLog({ actorId: user.id, action: "tournament.update", entityType: "tournament", entityId: id, before: existing, after: update, reason: body.reason ?? null });
