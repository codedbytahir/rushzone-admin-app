...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { user } = await requireAdmin(req, "result.correct");
    ...
    const { data: newId, error } = await admin.rpc("correct_result", { p_result_id: resultId, p_new_kills: newKills, p_new_placement: newPlacement, p_new_points: newPoints, p_new_prize: newPrize, p_corrected_by: user.id, p_reason: reason } as any);
    if (error) return withCors(req, jsonError("INTERNAL" as any, error.message, 500));
    await writeAuditLog({ actorId: user.id, action: "result.correct", entityType: "match_result", entityId: resultId, reason, before: existing, after: { kills: newKills, placement: newPlacement, points: newPoints, prize_coins: newPrize } });
