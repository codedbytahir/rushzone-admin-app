...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await admin.schema("app").from("settings").upsert({ key: "streak_config", value: config, updated_by: user.id } as any);
    await writeAuditLog({ actorId: user.id, action: "streak.config.update", entityType: "settings", entityId: "streak_config", after: config });
