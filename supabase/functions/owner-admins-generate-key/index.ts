...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (existing) {
      ...
      await admin.schema("admin").from("security_credentials").insert({ assignment_id: assignmentId, key_hash: hash, status: "active", key_version: 1, issued_by: user.id, failed_attempts: 0 } as any);
    }
    await admin.schema("admin").from("sessions").update({ revoked_at: new Date().toISOString() }).eq("assignment_id", assignmentId);
    await writeAuditLog({ actorId: user.id, action: "admin_super_key_generated", entityType: "assignment", entityId: assignmentId, after: { key_version: nextVersion } });
