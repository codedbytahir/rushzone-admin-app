...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { user } = await requireAdmin(req, "withdrawal.pay");
    ...
    if (wd.amount_coins >= threshold) {
      if (!secondReviewer && wd.reviewed_by !== user.id) {
      ...
      if (wd.reviewed_by === user.id && !secondReviewer) return withCors(req, jsonError("FORBIDDEN" as any, "Creator cannot be payer above threshold", 403));
    }
    const key = `withdraw_paid:${id}`;
    const { error: finErr } = await admin.rpc("wallet_finalize_held", { p_profile_id: wd.profile_id, p_amount: wd.amount_coins, p_type: "withdrawal_paid", p_reference_type: "withdrawal", p_reference_id: id, p_idempotency_key: key, p_created_by: user.id } as any);
    if (finErr) return withCors(req, jsonError("INTERNAL" as any, finErr.message, 500));
    await admin.schema("app").from("withdrawal_requests").update({ status: "paid", payout_ref: payoutRef, reviewed_by: wd.reviewed_by ?? user.id, second_reviewer: secondReviewer ?? user.id, updated_at: new Date().toISOString() }).eq("id", id);
    ...
    await writeAuditLog({ actorId: user.id, action: "withdrawal.paid", entityType: "withdrawal", entityId: id, after: { payout_ref: payoutRef, amount: wd.amount_coins } });
