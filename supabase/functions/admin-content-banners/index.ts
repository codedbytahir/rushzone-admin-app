...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "create") {
      ...
      await writeAuditLog({ actorId: userData.user.id, action: "content.banner.create", entityType: "banner", entityId: data.id });
    ...
    if (action === "update") {
      ...
      await writeAuditLog({ actorId: userData.user.id, action: "content.banner.update", entityType: "banner", entityId: id, after: upd });
    ...
    if (action === "delete") {
      ...
      await writeAuditLog({ actorId: userData.user.id, action: "content.banner.delete", entityType: "banner", entityId: id });
      return withCors(req, new Response(JSON.stringify({ ok: true }), { headers: { "Content-Type": "application/json", ...corsHeaders(req) } }));
    }
