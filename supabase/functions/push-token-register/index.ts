...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const token = body.token ?? body.expo_push_token ?? body.fcm_token;
    const platform = body.platform ?? "android";
    if (!token) return withCors(req, jsonError("VALIDATION_ERROR" as any, "token required", 400));
    await admin.schema("app").from("push_tokens").upsert({ profile_id: userData.user.id, token, platform, device: body.device ?? null } as any, { onConflict: "profile_id,token" });
